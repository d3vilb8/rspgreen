...
const Header = ({ user, notif }) => {
    ...
    console.log(props);
    return (
        <div className='w-full bg-[#fff] shadow-md p-2'>
            {props.auth.user.roles[0]?.name !== "admin" && props.auth.user.roles[0]?.name !== "customer" && !props.auth.user.is_exe_login && (
                <div className="fixed flex items-center justify-center inset-0 z-50 bg-black/30 backdrop-blur-sm">
                    <div className="w-full py-4 bg-white px-6 rounded">
