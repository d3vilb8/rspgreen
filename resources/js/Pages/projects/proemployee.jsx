...
const Task = ({ user, tasks, user_type, notif, taskcategory, employee,projects }) => {
                ...
                <div className='flex justify-between'>
                    ...
                    <div className='flex'>
                        <div className='grid p-2 mt-2 text-black underline rounded-lg place-items-center'>
                            <Link href='projects-create'>Add New</Link>
                        </div>
                        {/* <div className='grid p-2 mt-2 text-black underline rounded-lg place-items-center'>
    ...
    <thead className='bg-[#0A1B3F] text-white'>
        <tr>
            ...
            <th className="px-4 py-2 border border-gray-300">Estimate Hours</th>
    ...
    <tbody>
        {currentTasks.length > 0 ? (
            tasks.map((project, projectIndex) => {
                // Filter the tasks of the current project based on the pagination.
                const projectTasks = currentTasks.filter(task => task.project_id === project.id);

                // Skip projects that don't have any tasks after filtering.
                if (projectTasks.length === 0) return null;

                return projectTasks.map((task, taskIndex) => (
                    <tr key={task.id}>
                        {/* Show project name only in the first row of its tasks */}
                        {taskIndex === 0 && (
                           <>
                             <td
                                rowSpan={projectTasks.length}
                                className="px-4 py-2 border border-gray-300 text-[0.9rem] font-bold"
                            >
                                {project.title}
                            </td>
                             <td
 rowSpan={projectTasks.length}
                        ...
                        {/* <td className="flex px-4 py-2 space-x-2 border border-gray-300">
                            ...
                            />
                            <button
                                onClick={() => handleCopyClick(project.title)}
