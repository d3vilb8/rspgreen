...
const Designation = ({ user, notif, user_type, designations }) => {
    ...
    const openModal = (branch = null) => {
        setIsModalOpen(true);
        setData('ds_name', branch ? branch.ds_name : ''); // Set form field for editing
    ...
    const closeModal = () => {
        ...
        reset('ds_name'); // Reset the form field
    };
    ...
    const handleCreateOrUpdateBranch = (e) => {
        ...
        if (selectedBranchId) {
            // Update branch
            post(`/designations-up/${selectedBranchId}`, {
                onSuccess: () => {
    ...
    if (confirm('Are you sure you want to delete this branch?')) {
        try {
            ...
            notyf.error('Designation deleted successfully!'); // Show error message
    ...
    <table className='min-w-full border border-gray-300'>
        ...
        <tbody className=''>
            ...
            <Modal show={isModalOpen} onClose={closeModal}>
                <div className="p-6">
                    <h2 className="text-lg font-bold">{selectedBranchId ? 'Edit Designation' : 'Create New Designation'}</h2>
                    {errors.ds_name && <p className="text-red-600">{errors.ds_name}</p>}
                    <form onSubmit={handleCreateOrUpdateBranch} className="mt-4">
                        <label className="block mb-2">
                            ...
                            <input
                                type="text"
                                value={data.ds_name}
                                onChange={(e) => setData('ds_name', e.target.value)}
